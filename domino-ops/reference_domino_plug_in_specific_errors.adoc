...
Standardmäßig schlägt eine Datensicherung fehl, wenn eine oder mehrere Datenbanken inkonsistent oder beschädigt sind. Sie können das Plug-in zwingen, die Datensicherung trotz Fehlern fortzusetzen, wie in Schritt link:task_using_the_gui_to_create_a_configuration_file.html["Aufgabe mit der gui zum Erstellen einer Konfigurationsdatei"] beschrieben.
...
Das Domino Plug-in kann nicht mit SNAP_TIMESTAMP_ONLY = N. arbeiten SNAP_TIMESTAMP_ONLY sollte auf Y. gesetzt werden.
 a| 
Weitere Informationen finden Sie unter Schritt link:task_using_the_gui_to_create_a_configuration_file.html["Aufgabe: Mit der GUI eine Konfigurationsdatei erstellen"].
