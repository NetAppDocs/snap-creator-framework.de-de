...
Sie sollten das Changeinfo-Volume bereits als zu sicherndes Volume identifiziert haben, als Sie die Konfigurationsdatei erstellt haben. Weitere Informationen finden Sie in Schritt link:task_using_the_gui_to_create_a_configuration_file.html["Aufgabe mit der gui zum Erstellen einer Konfiguration"].
