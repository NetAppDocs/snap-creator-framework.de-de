...
Die Wiederherstellung von Volumes und einzelnen Dateien erfolgt in der Regel in unterschiedlichen Zielverzeichnissen. Das Zielverzeichnis wird im Parameter „Datenwiederherstellungsverzeichnis“ in der Sicherungs- und Wiederherstellungskonfigurationsdatei (siehe Schritt link:task_using_the_gui_to_create_a_configuration_file.html["Aufgabe mit der gui zum Erstellen einer Konfigurationsdatei"]) angegeben:
